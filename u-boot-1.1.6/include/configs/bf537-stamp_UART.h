...
#define CFG_LONGHELP			1
 /*#define CONFIG_BOOTDELAY	        1  -1 disable autoboot*/
#define CONFIG_BOOT_RETRY_TIME		-1	/* Enable this if bootretry required, currently its disabled */
...
#define CONFIG_EXTRA_ENV_SETTINGS                               \
	"boot=tftp 2000000 linux; bootelf 2000000\0" \        
	"ramargs=setenv bootargs root=/dev/mtdblock0 rw console=ttyBF0,57600\0"      \
        ...
        ":$(hostname):eth0:off\0"                               \
        "rambooot=tftpboot $(loadaddr) linux;"                     \
        "run ramargs;run addip;bootelf\0"                       \
        "nfsboot=tftpboot $(loadaddr) linux;"                     \
        "run nfsargs;run addip;bootelf\0"                       \
        "flashboot=bootm 0x20100000\0"                          \
        "update=tftpboot $(loadaddr) u-boot.bin\0"                 \
	"updatedhcp=dhcp;set serverip 172.18.128.16; tftp 1000000 u-boot.bin; go 1000000\0"\
        ...
        "bootdhcp=dhcp;set serverip 172.18.128.16; tftp 2000000 linux; bootelf 2000000\0"\
        "bootudhcp=dhcp;set serverip 172.18.128.16; tftp 2000000 uImage; bootm 2000000;"\
	""
#else
#define CONFIG_EXTRA_ENV_SETTINGS                               \
        "ramargs=setenv bootargs root=/dev/mtdblock0 rw console=ttyBF0,57600\0" \
